...
import { MdOutlinePhoneIphone, MdDiscount, MdAccountCircle, MdChevronLeft, MdChevronRight } from "react-icons/md";
...
export default function detail_product() {
    ...
    const [popup, setPopup] = useState<{
        visible: boolean;
        message: string;
        type: "success" | "error" | "warning" | ""; // thêm warning cho xác nhận
        //onSubmit?: () => void;  nếu popup là xác nhận
    }>({
        visible: false,
        message: "",
        type: "",
    ...
    const [sl, setQuantity] = useState(0);

    const increase_quantity = () => {
        const soluongtoida = phone?.quantity ?? 0;
        sl == 0;
        setQuantity((r) => Math.min(sl + 1, soluongtoida));
    }

    const descrease_quantity = () => {
        if (sl > 0) {
            setQuantity((r) => Math.max(sl - 1, 0))
        }
    }
    ...
    const handleAddToCart = async () => {
        try {
            ...
            if (result) {
                setPopup({ visible: true, message: "Thêm vào giỏ hàng thành công!", type: "success" });
            }
        } catch (err) {
            console.error(err);
    ...
    const discount = phone?.percent ?? 0;
        ...
        <section className={styles.detail_product}>
                ...
                <div className={styles.gridContainer}>
                        ...
                        <div className={styles.slideshow_container}>
                            <div className={styles.main_image_container}>
                                {phone?.images && phone.images.length > 1 && (
                                    <button className={styles.nav_btn_left} onClick={prevImage}>
                                        <MdChevronLeft />
                                    </button>
                                )}

                                <Image
                                    src={phone?.images?.[currentImageIndex] ?? mota_S25}
                                    width={500}
                                    height={500}
                                    alt={phone?.name_product ?? "Product Image"}
                                    className={styles.main_image}
                                />

                                {phone?.images && phone.images.length > 1 && (
                                    <button className={styles.nav_btn_right} onClick={nextImage}>
                                        <MdChevronRight />
                                    </button>
                                )}
                            </div>
                            ...
                            {phone?.images && phone.images.length > 1 && (
                                <div className={styles.thumbnail_container}>
                                    {phone.images.map((img, index) => (
                                        <div
                                            key={index}
                                            className={`${styles.thumbnail} ${index === currentImageIndex ? styles.active_thumbnail : ""}`}
                                            onClick={() => setCurrentImageIndex(index)}
                                        >
                                            <Image src={img} width={80} height={80} alt={`Thumbnail ${index}`} />
                                        </div>
                                    ))}
                                </div>
                            )}
                            ...
                            <table >
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td className="border p-2">{phone?.brand ?? "Loading..."}</td>
                                    </tr>
                                    {/* <tr>
                                    ...
                                    <tr>
                                        ...
                                        <td className="border p-2">{phone?.color ?? "Loading..."}</td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        ...
                                        <td className="border p-2">{phone?.operating_system ?? "Loading..."}</td>
                                    </tr>
                                    <tr>
                                        <td className="border p-2 font-semibold" style={{ backgroundColor: '#d0d0d0ff' }}>Độ phân giải màn hình</td>
                                        <td className="border p-2">{phone?.resolution ?? "Loading..."}</td>
                                    </tr>
                                    <tr>
                                        <td className="border p-2 font-semibold" style={{ backgroundColor: '#d0d0d0ff' }}>Tần số quét màn hình</td>
                                        <td className="border p-2">{phone?.screen_frequency ?? "Loading..."}</td>
                                    </tr>
                                    {/* <tr>
                    ...
                    <div className={styles.item}>
                        ...
                        <div className={styles.discount}>
                            ...
                            <div>
                                <span style={{ color: '#A70000', fontWeight: 'bold' }}>
                                    {phone?.name_promotion ?? "Chưa có chương trình ưu đãi"}
                                </span>
                                <p>
                                    {phone?.describe_promotion ?? "Chưa có mô tả chương trình ưu đãi"}
                                </p>
                            </div>
                            <div style={{ fontSize: '50pt', fontWeight: 'bold', color: '#A70000' }}>{phone?.percent ?? "Loading..."}%</div>
                        ...
                        <div className={styles.button}> {/*nút đặt + nút thêm vào giỏ hàng */}
                            ...
                            <div className={styles.stock_info}>
                                Còn lại: <strong>{phone?.quantity ?? 0}</strong> sản phẩm
                            </div>
                            <div className={styles.quantity}> {/*Điều chỉnh số lượng */}
                                <div className={styles.icon_quantity} onClick={increase_quantity}><CiSquarePlus /></div>
                                <span style={{ fontSize: '20pt' }}>{sl}</span>
                                <div className={styles.icon_quantity} onClick={descrease_quantity}><CiSquareMinus /></div>
                            </div>
                    ...
                    <div className={`${styles.item} ${styles.videoads}`}>
                        ...
                        <iframe width="760" height="415"
                            src={phone?.video ?? "https://www.youtube.com/embed/FQ8OET3__2Y?si=Ski5N-HGVwu-0kPU"}
            ...
            {popup.visible && (
                <PopUp
                    ...
                    type={popup.type as "" | "success" | "error" | "warning"}
                    onClose={() => {
