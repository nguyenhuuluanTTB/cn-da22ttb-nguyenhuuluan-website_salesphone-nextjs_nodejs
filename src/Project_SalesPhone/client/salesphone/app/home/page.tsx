...
import Sales50 from './api/getSales50.js';
import fetchPhoneUnder5Tr from './api/getPhoneUnder5Tr.js';
...
export default function Home() {
    ...
    const [phone50, setphone50] = useState<Phone[]>([]);
    const [phoneUnder5Tr, setUnder5Tr] = useState<Phone[]>([]);
    const [loading, setLoading] = useState(true);
    const [promotions, setPromotions] = useState<any[]>([]);
    const [selectedPromo, setSelectedPromo] = useState<any>(null);

    useEffect(() => {
        fetch('http://localhost:5000/api/product/promotions')
            .then(res => res.json())
            .then(data => {
                if (data.success) {
    ...
    async function fetchSales50() {
        try {
            const token = localStorage.getItem('token');
            const result = await Sales50(token);
            console.log('50: ', result.data);
            if (!result) {
                console.error('http error!');
            }
    ...
    async function Duoi5Tr() {
        try {
            const token = localStorage.getItem('token');

            const result = await fetchPhoneUnder5Tr(token);
            if (!result) {
                console.error('Http error!');
            }
            ...
            <div className={styles.container}> {/*Đây là thẻ container chứa toàn bộ nội dung bên trong (hai bên mép thu lại)*/}
                ...
                <div className={styles.tab_container}> {/*tab_container */}
                    ...
                    <div className={styles.tab_header}> {/*tab_header */}
                        ...
                        <button className={activeTab === "tab2" ? styles.active : ""} onClick={() => setActiveTab("tab2")}>DEAL SỐC GIẢM ĐẾN 50%</button>

                        <button className={activeTab === "tab3" ? styles.active : ""} onClick={() => setActiveTab("tab3")}>SẢN PHẨM GIÁ DƯỚI 5 TRIỆU</button>
                    </div>
                    ...
                    <div className={styles.tab_content}> {/*tab_content */}
                            ...
                            <div className={styles.tab_item}>
                                <div className={styles.grid}>
                                    {phones.map((phone) => {
                                        ...
                                        return (
                                            <Link key={phone.phone_id} href={`/home/detail_product/${phone.product_code}`} style={{ textDecoration: 'none' }}>
                                                <div key={phone.phone_id} className={`${styles.card_a} ${styles.card}`} >
                                                    <div className={styles.tragop_discount}> {/*cho phần giảm giá và trả góp */}
                                                        <div className={styles.tragop}>Trả góp 0%</div>
                                                        {phone.percent > 0 && (
                                                            <div className={styles.discount_percent}>Giảm giá {phone.percent}%</div>
                        ...
                        {activeTab === "tab2" && (<div className={styles.tab_item}>
                            <div className={styles.grid}>
                                {phone50.map((phone) => {
                        ...
                        {activeTab === "tab3" && (<div className={styles.tab_item}>
                            <div className={styles.grid}>
                ...
                <div className={styles.tab_container} style={{ marginTop: '2rem' }}>
                    <h2 style={{ color: '#A70000', fontSize: '1.5rem', fontWeight: 'bold', marginTop: '1rem', marginBottom: '1.5rem', textAlign: 'center', textTransform: 'uppercase' }}>
                        Chương trình khuyến mãi
                    </h2>

                    <div className={styles.promotion_grid}>
                        {promotions.length > 0 ? promotions.map((promo, index) => (
                            <div key={promo.id_promotion} className={styles.promo_banner}
                                onClick={() => setSelectedPromo(promo)}
                                style={{
                                    backgroundImage: promo.image_pro ? `url(${promo.image_pro})` :
                                        (index % 3 === 0 ? 'linear-gradient(135deg, #FF9A9E 0%, #FECFEF 100%)' :
                                            index % 3 === 1 ? 'linear-gradient(135deg, #a18cd1 0%, #fbc2eb 100%)' :
                                                'linear-gradient(135deg, #84fab0 0%, #8fd3f4 100%)')
                                }}>
                                <h3>{promo.name_promotion}</h3>
                                <p>{promo.describe_promotion || ''}</p>
                            </div>
                        )) : (
                            <div className={styles.promo_banner} style={{ background: '#f0f0f0', gridColumn: '1/-1', textAlign: 'center' }}>
                                <p style={{ color: '#333' }}>Đang cập nhật chương trình khuyến mãi...</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                {selectedPromo && (
                    <div className={styles.modalOverlay} onClick={() => setSelectedPromo(null)}>
                        <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
                            <button className={styles.closeBtn} onClick={() => setSelectedPromo(null)}>&times;</button>
                            <h2>{selectedPromo.name_promotion}</h2>
                            {selectedPromo.image_pro && (
                                <img src={selectedPromo.image_pro} alt={selectedPromo.name_promotion} />
                            )}
                            <div dangerouslySetInnerHTML={{ __html: selectedPromo.describe_promotion || 'Đang cập nhật mô tả...' }} />

                            {selectedPromo.start_date && selectedPromo.end_date && (
                                <p style={{ marginTop: '20px', fontStyle: 'italic', color: '#666', borderTop: '1px solid #eee', paddingTop: '10px' }}>
                                    📅 Thời gian: {new Date(selectedPromo.start_date).toLocaleDateString('vi-VN')} - {new Date(selectedPromo.end_date).toLocaleDateString('vi-VN')}
                                </p>
                            )}
                        </div>
                    </div>
