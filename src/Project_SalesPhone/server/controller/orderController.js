    ...
    try {
        ...
        try {
            ...
            for (const product of products) {
                ...
                // 3. Giảm số lượng sản phẩm trong kho
                await sequelize.query(
...
/**
 * Đồng bộ tất cả đơn hàng chưa hoàn thành với GHN
 * API này có thể chạy định kỳ bằng cron job hoặc scheduler
 */
exports.syncAllPendingOrders = async (req, res) => {
    try {
        console.log('========== SYNC ALL PENDING ORDERS ==========');

        // Lấy tất cả đơn hàng có mã GHN và chưa hoàn thành
        const [orders] = await sequelize.query(
            `SELECT id, ghn_order_code, status, payment_status
             FROM orders 
        ...
        for (const order of orders) {
            try {
                console.log(`Syncing order ${order.id} (${order.ghn_order_code})...`);

                // Lấy thông tin từ GHN
                const ghnResult = await getGHNOrderInfo(order.ghn_order_code);

                if (!ghnResult.success) {
                    console.warn(`  ⚠️ Failed to get info: ${ghnResult.message}`);
                    errorCount++;
                    results.push({
                        orderId: order.id,
                        success: false,
                ...
                if (statusUpdate.status !== order.status || statusUpdate.paymentStatus !== order.payment_status) {
                    await sequelize.query(
                        `UPDATE orders 
                         SET status = ?, 
                             payment_status = ?,
                    ...
                    console.log(`  ✅ Updated: ${order.status} → ${statusUpdate.status}`);
                    successCount++;
                    results.push({
                        orderId: order.id,
                        success: true,
                    ...
                    console.log(`  ℹ️ No change: ${order.status}`);
                    results.push({
                        orderId: order.id,
                        success: true,
                        noChange: true,
                ...
                console.error(`  ❌ Error syncing order ${order.id}:`, orderError.message);
                errorCount++;
                results.push({
                    orderId: order.id,
                    success: false,
        ...
        console.log(`Total: ${orders.length} | Success: ${successCount} | Errors: ${errorCount}`);
