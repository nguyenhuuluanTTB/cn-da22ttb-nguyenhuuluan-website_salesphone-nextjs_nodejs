...
async function getAvailableModel() {
  // Danh sách models ưu tiên (stable models)
  ...
  try {
    ...
    // Tìm model trong danh sách ưu tiên
    for (const preferredModel of preferredModels) {
      ...
      if (model) {
        console.log('✅ Using stable model:', model.name);
        return model.name;
      }
    }
    
    // Nếu không tìm thấy, dùng model đầu tiên hỗ trợ generateContent
    const anyModel = res.data.models?.find(m => 
      m.supportedGenerationMethods?.includes("generateContent") &&
      m.name.includes('gemini')
    );
    
    if (anyModel?.name) {
      console.log('✅ Using available model:', anyModel.name);
...
Dữ liệu sản phẩm:
${JSON.stringify(phonesWithImages.slice(0, 6), null, 2)}
...
`;
      ...
      // Fallback response với formatPhonesHTML
      return res.json({
