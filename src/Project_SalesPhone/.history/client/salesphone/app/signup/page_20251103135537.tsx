...
const [otpPopup, setOtpPopup] = useState({
  ...
  otp: "",
    ...
    const handleOpenOtpPopup = () => {
      ...
      const handleOtpSubmit = async () => {
        try {
            console.log({ name, email, password, otp: otpPopup.otp }); // Kiểm tra giá trị
  ...
  <PopUp
    ...
    type={""} // Adjusted to match supported types
    onClose={handleCloseOtpPopup}
    onSubmit={handleOtpSubmit}
    inputValue={otpPopup.otp}
    onInputChange={(e: React.ChangeEvent<HTMLInputElement>) => setOtpPopup({ ...otpPopup, otp: e.target.value })}
  />
)}
