...
export default function Home () {
    ...
    const articles = [
        {
            ...
            excerpt: "Apple chính thức công bố dòng iPhone 16 Pro với chip A18 Pro mạnh mẽ, camera cải tiến và pin lâu hơn...",
            date: "12/11/2024",
            like: 120,
            cmt: 45
        },
    ...
    // Sidebar bài viết nổi bật (giữ nguyên)
    const sidebarPosts = [
                ...
                <section className={styles.wrapper_baiviet}>
                    ...
                    <div className={styles.tabContainer_baiviet}>
                        {tabs.map((tab) => (
                            <button
                                key={tab.key}
                                className={`${styles.tabBtn} ${activeTab_article === tab.key ? styles.active : ""}`}
                                onClick={() => setActiveTab_article(tab.key)}
                            >
                                {tab.label}
                            </button>
                        ))}
                    ...
                    <div className={styles.content}>
                        {/* Left - Danh sách bài viết theo tab */}
                        <div className={styles.sliderArea}>
                            <div className={styles.tagFeatured}>🔥 Nổi bật</div>
                            <div className={styles.articlesList}>
                                {articles
                                    .filter(article => activeTab_article === "all" || article.type === activeTab_article)
                                    .map(article => (
                                        <div key={article.id} className={styles.sliderCard}>
                                            <Image src={article.image} alt={article.title} width={300} height={150} />
                                            <div className={styles.sliderInfo}>
                                                ...
                                                <div style={{display: 'flex', flexDirection: 'row', justifyContent: 'space-between'}}>
                                                    ...
                                                    <div style={{display: 'flex', flexDirection: 'row', gap: '10px'}} className={styles.social}>
                                                        <span style={{display: 'flex', flexDirection: 'row', alignItems: 'center'}}><FaRegHeart />&nbsp; {article.like}</span>
                                                        <span style={{display: 'flex', flexDirection: 'row', alignItems: 'center'}}><FaRegComment/>&nbsp;{article.cmt}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                        ...
                        <div className={styles.sidebar}>
                            {sidebarPosts.map((post, i) => (
                                <div key={i} className={styles.sidebarCard}>
                                    <div className={styles.tag}> <Image src={post.tag} alt={post.title} width={70} height={70}/> </div> 
