...
export default function Home () {
    ...
    // Sidebar bài viết nổi bật lấy từ articlesData
    const sidebarPosts = articles.slice(1, 5); // lấy 4 bài tiếp theo làm sidebar
            ...
            <div className={styles.container}> {/*Đây là thẻ container chứa toàn bộ nội dung bên trong (hai bên mép thu lại)*/}
                ...
                <section className={styles.wrapper_baiviet}>
                    ...
                    <div className={styles.content}>
                        {/* Left - Slider: bài viết nổi bật, click vào chuyển trang chi tiết */}
                        <div className={styles.sliderArea}>
                            <div className={styles.tagFeatured}>🔥 Nổi bật</div>
                            <div className={styles.slider}>
                                {/* Lấy bài đầu tiên làm nổi bật */}
                                <Link href={`/home/article/${articles[0].id}`}>
                                    <a style={{display: 'block', borderRadius: 16, overflow: 'hidden'}}>
                                        <div className={styles.sliderCard}>
                                            <Image src={articles[0].image} alt={articles[0].title} width={830} height={400} />
                                            <div className={styles.sliderInfo} style={{position: 'absolute', bottom: 0, left: 0, right: 0, color: '#fff', background: 'rgba(0,0,0,0.35)', padding: '1.5rem'}}>
                                                <h3 style={{fontSize: '2rem', marginBottom: 8}}>{articles[0].title}</h3>
                                                <p>{articles[0].excerpt}</p>
                                                <div style={{display: 'flex', flexDirection: 'row', justifyContent: 'space-between', marginTop: 8}}>
                                                    <span>{articles[0].date}</span>
                                                    <span>❤️ {articles[0].like} 💬 {articles[0].cmt}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </a>
                        ...
                        {/* Right - Sidebar: các bài viết khác, click vào chuyển trang chi tiết */}
                        <div className={styles.sidebar}>
                            {sidebarPosts.map((post, i) => (
                                <Link key={post.id} href={`/home/article/${post.id}`}>
                                    <a className={styles.sidebarCard} style={{display: 'flex', alignItems: 'center', gap: 12, textDecoration: 'none'}}>
                                        <div className={styles.tag}> <Image src={post.image} alt={post.title} width={70} height={70}/> </div>
                                        <div className={styles.sidebarContent}>
                                            <div style={{marginBottom: '5px'}}>
                                                <span style={{padding: '3px 5px', backgroundColor: '#decdcdff', color: '#A70000', fontSize: '8pt', borderRadius: '3px'}}>{post.type === 'review' ? 'Đánh giá' : post.type === 'tips' ? 'Thủ thuật' : post.type === 'compare' ? 'So sánh' : 'Tin tức'}</span>
                                            </div>
                                            <h4 style={{fontSize: '1rem', margin: 0}}>{post.title}</h4>
                                            <p className={styles.iconLine} style={{margin: 0, color: '#888', fontSize: 13}}>< FaRegHeart /> {post.like}  <FaRegComment/> {post.cmt}</p>
                                        </div>
                                    </a>
                                </Link>
                            ))}
