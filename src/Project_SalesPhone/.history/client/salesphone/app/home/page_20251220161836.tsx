...
export default function Home () {
    ...
    // import articles from './articlesData';
    const sidebarPosts = articles.slice(1, 5); // 4 bài tiếp theo làm sidebar
                                ...
                                <section className={styles.wrapper_baiviet}>
                                    <div className={styles.header_baiviet}>
                                        <div className={styles.container_baiviet}>
                                            ...
                                            <a className={styles.viewAll} href="/home/article">Xem tất cả →</a>
                                        </div>
                                    </div>
                                    <div className={styles.content}>
                                        ...
                                        <div className={styles.sliderArea}>
                                            ...
                                            <div className={styles.slider}>
                                                <a href={`/home/article/${articles[0].id}`} style={{display:'block',borderRadius:12,overflow:'hidden',textDecoration:'none',color:'inherit'}}>
                                                    <div className={styles.sliderCard}>
                                                        <img src={typeof articles[0].image === 'string' ? articles[0].image : undefined} alt={articles[0].title} width={830} height={200} style={{width:'100%',objectFit:'cover'}} />
                                                        <div className={styles.sliderInfo}>
                                                            <h3>{articles[0].title}</h3>
                                                            <p>{articles[0].excerpt}</p>
                                                            <div style={{display: 'flex', flexDirection: 'row', justifyContent: 'space-between'}}>
                                                                <div className={styles.meta}><span>{articles[0].date}</span></div>
                                                                <div style={{display: 'flex', flexDirection: 'row', gap: '10px'}} className={styles.social}>
                                                                    <span style={{display: 'flex', flexDirection: 'row', alignItems: 'center'}}><FaRegHeart />&nbsp; {articles[0].like}</span>&nbsp; <span style={{display: 'flex', flexDirection: 'row', alignItems: 'center'}}><FaRegComment/>&nbsp;{articles[0].cmt}</span>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                        ...
                                        <div className={styles.sidebar}>
                                            {sidebarPosts.map((post, i) => (
                                                <a key={post.id} href={`/home/article/${post.id}`} className={styles.sidebarCard} style={{display:'flex',alignItems:'center',gap:12,textDecoration:'none',color:'inherit'}}>
                                                    <div className={styles.tag}> <img src={typeof post.image === 'string' ? post.image : undefined} alt={post.title} width={70} height={70}/> </div>
                                                    <div className={styles.sidebarContent}>
                                                        <div style={{marginBottom: '5px'}}><span style={{padding: '3px 5px', backgroundColor: '#decdcdff', color: '#A70000', fontSize: '8pt', borderRadius: '3px'}}>{post.type === 'review' ? 'Đánh giá' : post.type === 'tips' ? 'Thủ thuật' : post.type === 'compare' ? 'So sánh' : 'Tin tức'}</span></div>
                                                        <h4>{post.title}</h4>
