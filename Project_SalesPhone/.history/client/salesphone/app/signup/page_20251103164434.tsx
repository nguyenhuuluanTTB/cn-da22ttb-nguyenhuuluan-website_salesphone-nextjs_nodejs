...
const [otpPopup, setOtpPopup] = useState({
  ...
  otp: "",
        ...
        useEffect(() => {
            ...
            // prefer Next.js public env but fall back to common REACT_APP name if present
            const clientId = process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID || (process.env.REACT_APP_GOOGLE_CLIENT_ID as string) || '';
            console.log('GSI clientId (signup):', clientId);
            if (!clientId) {
                console.warn('Google client id not set for signup (NEXT_PUBLIC_GOOGLE_CLIENT_ID or REACT_APP_GOOGLE_CLIENT_ID)');
                return;
            }
            ...
            const tryInit = () => {
                ...
                if ((window as any).google && (window as any).google.accounts && (window as any).google.accounts.id) {
                    try {
                        ...
                        // render a button into #gsi_btn (if present) to match LoginPage behavior
                        const btnContainer = document.getElementById('gsi_btn');
                        if (btnContainer) {
                            try {
                        ...
                        console.error('GSI initialize error (signup):', err);
